...
app.get('/generate', requireRole('admin', 'brand', 'brand_staff'), (req, res) => {
  ...
  const factories = scope === null
    ...
    : factoriesForBrand(scope).map(factory => ({ id: factory.id, name: factory.name, brand_id: factory.brand_id }));
...
app.get('/factory', requireRole('admin', 'factory', 'brand', 'brand_staff'), (req, res) => {
  ...
  let factories;
  if (role === 'admin') factories = db.prepare('SELECT id,name,brand_id FROM factories ORDER BY name,id').all();
  else if (role === 'factory') factories = db.prepare('SELECT id,name,brand_id FROM factories WHERE id=?').all(operator.factory_id || 0);
  else factories = factoriesForBrand(operator.brand_id || 0);
...
function resolvePackingFactory(req, product, requestedFactoryId, box = null) {
  const operator = currentUser(req) || {};
  let factoryId = operator.role === 'factory'
    ? asId(operator.factory_id)
    : (asId(requestedFactoryId) || asId(box?.receipt_factory_id) || (operator.role === 'admin' ? 0 : asId(product?.factory_id)));
  if (!factoryId) return { error: operator.role === 'admin' ? '平台管理员装箱必须先选择工厂' : '请选择本次入库工厂' };
  const factory = db.prepare('SELECT id,name,brand_id FROM factories WHERE id=?').get(factoryId);
  if (!factory) return { error: '所选工厂不存在，请刷新后重试' };
  ...
  if (box?.receipt_factory_id && Number(box.receipt_factory_id) !== factoryId && Number(box.item_count || 0) > 0) {
    return { error: '该箱已经开始扫码，不能更换入库工厂' };
  }
...
app.post('/api/factory/pack/start', requireRole('admin', 'factory', 'brand', 'brand_staff'), (req, res) => {
  ...
  const requestedFactoryId = asId(req.body.factory_id);
  ...
  if (box.product_id) {
    ...
    if (resolved.error) return res.status(403).json({ success: false, code: 'PACKING_FACTORY_FORBIDDEN', msg: resolved.error });
  }
...
app.post('/api/factory/pack/scan-nobox', requireRole('admin', 'factory', 'brand', 'brand_staff'), receiptIntegrity.atomic((req, res) => {
  ...
  const requestedBrandId = asId(req.body.brand_id);
  if (requestedBrandId && Number(product.brand_id) !== requestedBrandId) return res.status(409).json({ success: false, msg: '产品与所选品牌不一致' });
  const resolvedFactory = resolvePackingFactory(req, product, req.body.factory_id);
  if (resolvedFactory.error) return res.status(403).json({ success: false, code: 'PACKING_FACTORY_FORBIDDEN', msg: resolvedFactory.error });
...
app.get('/users', requireRole('admin', 'brand'), (req, res) => {
  ...
  const factories = scope === null
    ...
    : factoriesForBrand(scope).map(factory => ({ id: factory.id, name: factory.name }));
  const invitations = scope === null
    ? db.prepare(`
        SELECT i.id, i.code, i.role, i.note, i.status, i.used_by, i.used_at, i.created_at, b.name as brand_name, f.name as factory_name, u.display_name as user_name
        FROM invitations i LEFT JOIN brands b ON i.brand_id=b.id LEFT JOIN factories f ON i.factory_id=f.id LEFT JOIN users u ON i.used_by = u.username
...
app.get('/api/factories', requireRole('admin', 'brand'), (req, res) => {
  const scope = brandScope(req);
  const base = `SELECT DISTINCT f.id,f.name,f.contact,f.created_at,f.brand_id,b.name as brand_name
    FROM factories f LEFT JOIN brands b ON b.id=f.brand_id LEFT JOIN factory_brands linked ON linked.factory_id=f.id`;
  const factories = scope === null
    ? db.prepare(base + ' ORDER BY f.id DESC').all()
    : db.prepare(base + ' WHERE f.brand_id=? OR linked.brand_id=? ORDER BY f.id DESC').all(scope, scope);
