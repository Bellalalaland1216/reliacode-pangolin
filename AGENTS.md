...
- 单屏适配必须保留已确认的满幅背景和登录构图，不能把整张画面缩入容器后留下两侧竖条；背景铺满与表单适配分别处理。

## UI 设计规则协议

进行 UI 设计、前端调整、动画或视觉修复前，先阅读[UI 设计规则协议](UI设计规则协议.md)，并核对当前页面对应的参考图。该文件集中记录用户已确认的视觉偏好、修改边界、页面专属约定与验收要求；用户最新明确指示优先。后续维护完整规则以该文件为准，避免在多个副本中分别修改。
